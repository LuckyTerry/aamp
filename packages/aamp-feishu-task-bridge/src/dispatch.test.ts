...
test('buildFeishuTaskPromptRules includes complete handling and result rules', () => {
  ...
  assert.match(rules, /Feishu\/Lark Authorization Rules:/)
  assert.match(rules, /inspect the current granted user scopes/i)
  assert.match(rules, /hard capability boundary/i)
  assert.match(rules, /Do not run lark-cli auth login/i)
  assert.match(rules, /do not request additional OAuth scopes/i)
  assert.match(rules, /do not ask the user to grant new Feishu\/Lark permissions/i)
  assert.match(rules, /using only currently granted scopes/i)
  assert.match(rules, /do not include authorization commands or scope requests/i)
