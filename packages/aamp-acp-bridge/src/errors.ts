...
export class UserFacingBridgeError extends Error {
  readonly userMessage: string

  constructor(userMessage: string, options?: ErrorOptions) {
    super(userMessage, options)
    this.name = 'UserFacingBridgeError'
...
export function describeBridgeEventError(error: unknown): string {
  return error instanceof UserFacingBridgeError
    ? error.userMessage
