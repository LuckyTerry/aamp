...
export const BUILTIN_CLI_PROFILES: Record<string, CliProfileDefinition> = {
  claude: {
    ...
    command: 'claude',
    args: ['-p', '{{prompt}}', '--resume', '{{sessionKey}}'],
    timeoutMs: 1_800_000,
  },
  codex: {
    name: 'codex',
    ...
    command: 'codex',
    args: ['exec', '--session-id', '{{sessionKey}}', '--skip-git-repo-check', '{{prompt}}'],
    timeoutMs: 1_800_000,
  },
  coco: {
    name: 'coco',
    ...
    command: 'coco',
    args: ['-p', '{{prompt}}', '--yolo', '--resume', '{{sessionKey}}'],
    timeoutMs: 1_800_000,
  },
  gemini: {
    name: 'gemini',
    ...
    command: 'gemini',
    args: ['--prompt', '{{prompt}}', '--resume', '{{sessionKey}}'],
    timeoutMs: 1_800_000,
  },
  codem: {
    name: 'codem',
